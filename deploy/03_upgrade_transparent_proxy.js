...
module.exports = async ({getNamedAccounts}) => {
    ...
    if (!fs.existsSync(deploymentInfoPath)) {
        console.error("部署信息文件不存在，请先部署透明代理合约！");
        return;
    }
    const deploymentInfo = JSON.parse(fs.readFileSync(deploymentInfoPath, 'utf-8'));
    const proxyAddress = deploymentInfo.proxyAddress;
    console.log("读取到的透明代理合约地址：" + proxyAddress);

    // 连接代理合约
    const Box = await ethers.getContractFactory('Box');
    const boxProxy = await ethers.getContractAt('Box', proxyAddress);

    // 测试升级前的状态
    console.log("测试升级前的状态...")
    const valueBeforeUpgrade = await boxProxy.retrieve();
    console.log("升级前通过透明代理合约获取的值为：" + valueBeforeUpgrade);

    // 升级实现合约
    console.log("升级实现合约...")
    const BoxV2 = await ethers.getContractFactory('BoxV2');
    const boxProxyUpgraded = await upgrades.upgradeProxy(proxyAddress, BoxV2, {kind: 'transparent'});
    ...
    console.log("升级后通过透明代理合约获取的值为：" + valueAfterUpgrade);
    ...
    const upgradedDeploymentInfoPath = path.join(__dirname, 'transparent_proxy_upgraded_deployment_info.json');
...
module.exports.tags = ['TransparentProxyUpgrade'];
